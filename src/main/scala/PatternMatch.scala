object CreateClassByPatternMatch {

}

case class LogId(id: Int) {
  /**
   * scala> LogId(100) match { case LogId(id) => println(id) }
   * 100
   */
}
...
object UnapplyStudy extends App {
  ...
  printPatternMatched(new UnapplyStudy.User("taro", 17))
  printPatternMatched("Jiro@16")
  /**
   * scala> UnapplyStudy.printPatternMatched(new UnapplyStudy.User("taro", 17))
   * Name: taro, Age: 17
   *
   * scala> UnapplyStudy.printPatternMatched("Jiro@16")
   * Name: Jiro, Age: 16
   */
}